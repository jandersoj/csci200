...
public class LinkedBinarySearchTree<T extends Comparable<T>> extends LinkedBinaryTree<T> implements BinarySearchTreeADT<T> {
	...
	public boolean contains(T targetElement) {
		...
		//enter code here (leave above print statement)
	...
	public T find(T targetElement) throws ElementNotFoundException {
		...
		T ugh = traverseFind(root, targetElement);
		if(ugh == null) {
	...
	private T traverseFind(BinaryTreeNode<T> node, T target) {
		if(node != null) {
			
			Comparable<T> currData = (Comparable<T>)node.getData();
						
			if(target.compareTo(node.getData()) == 0){//visit node itself
				return node.getData();
				
			} else if(target.compareTo(node.getData()) > 0) {
				return traverseFind(node.getRight(), target); //visit right node
			} else if(target.compareTo(node.getData()) < 0) {
				return traverseFind(node.getLeft(), target); //visit left node
			}
